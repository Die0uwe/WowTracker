-- ============================================================
-- WowTracker Plugin Wrapper — Prey Tracker V3.5
-- KRITIEK: Kompas formule is heilig — NIET wijzigen
-- math.atan2(dx, -dy) + SetRotation(-relative + offset)
-- ============================================================
local addonName, addonTable = ...
local _ptWrap = CreateFrame("Frame")
_ptWrap:RegisterEvent("ADDON_LOADED")
_ptWrap:SetScript("OnEvent", function(self, event, name)
    ...
    WowTracker:RegisterPlugin({
        ...
        icon     = "🎯",
        enabled  = true,
        -- Geen eigen scan — prey tracker heeft eigen tick systeem
        buildUI  = function(cf)
            -- De prey UI heeft zijn eigen fullscreen HUD (/pt command)
            -- Dit panel toont status + knop om HUD te openen
            local title = cf:CreateFontString(nil,"OVERLAY"); title:SetFont("Fonts\\2002.ttf",13,"OUTLINE")
            title:SetPoint("TOPLEFT",14,-10); title:SetText("|cffbf00ff🎯 Prey Tracker — Midnight HUD|r")
            local info = cf:CreateFontString(nil,"OVERLAY"); info:SetFont("Fonts\\2002.ttf",11,"OUTLINE")
            info:SetPoint("TOPLEFT",14,-36)
            info:SetText("|cff888888De Prey Tracker HUD werkt als los kompas overlay.\nGebruik onderstaande knoppen om de HUD te beheren.|r")
            local function MakeBtn(label, cmd, yOff)
                local btn = CreateFrame("Button",nil,cf,"BackdropTemplate")
                btn:SetSize(200,34); btn:SetPoint("TOPLEFT",14,yOff)
                btn:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8x8",edgeFile="Interface\\Buttons\\WHITE8x8",edgeSize=1})
                btn:SetBackdropColor(0.08,0.03,0.15,1.0); btn:SetBackdropBorderColor(0.45,0.0,0.85,0.6)
                local lbl = btn:CreateFontString(nil,"OVERLAY"); lbl:SetFont("Fonts\\2002.ttf",12,"OUTLINE")
                lbl:SetPoint("CENTER"); lbl:SetText(label)
                btn:SetScript("OnClick",function() SlashCmdList[cmd] and SlashCmdList[cmd]("") or print("/pt niet beschikbaar") end)
                btn:SetScript("OnEnter",function(s) s:SetBackdropColor(0.16,0.05,0.26,1.0) end)
                btn:SetScript("OnLeave",function(s) s:SetBackdropColor(0.08,0.03,0.15,1.0) end)
            end
            MakeBtn("|cff00dfff⟳ Toggle Prey HUD|r",  "PREYTRACKER",  -80)
            MakeBtn("|cffccaa00⚙ Prey Instellingen|r", "PREYUI",       -124)
            MakeBtn("|cff44ff44▶ Test Modus|r",         "PREYTEST",     -168)

            -- Live status
            local statusLabel = cf:CreateFontString(nil,"OVERLAY"); statusLabel:SetFont("Fonts\\2002.ttf",11,"OUTLINE")
            statusLabel:SetPoint("TOPLEFT",14,-215)
            -- Probeer live data te tonen als preytracker actief is
            C_Timer.NewTicker(1.0, function()
                if not cf:IsShown() then return end
                local questID = C_QuestLog.GetActivePreyQuest and C_QuestLog.GetActivePreyQuest()
                if questID then
                    local title = C_QuestLog.GetTitleForQuestID and C_QuestLog.GetTitleForQuestID(questID) or "Onbekend"
                    statusLabel:SetText("|cff44ff44● Hunt actief:|r |cffccaa00"..tostring(title).."|r")
                else
                    statusLabel:SetText("|cff666666● Geen actieve Prey Hunt|r")
                end
            end)
        end,
        onEnable  = function() end,
        onDisable = function()
            -- Verberg HUD als die open is
            if _G["DT_PreyFrame"] then _G["DT_PreyFrame"]:Hide() end
        end,
    })
end)
-- ============================================================
