-- ============================================================
-- WowTracker Plugin Wrapper — Registry v8.1.0
-- Registreert de Registry in de nieuwe WowTracker Plugin API.
-- De bestaande XL Registry frame code blijft 100% intact.
-- Voeg dit BOVENAAN DT_Registry.lua toe (voor de if DelveTracker block)
-- ============================================================

local addonName, addonTable = ...

-- Wacht tot WowTracker core geladen is, registreer dan de plugin
local _regWrap = CreateFrame("Frame")
_regWrap:RegisterEvent("ADDON_LOADED")
_regWrap:SetScript("OnEvent", function(self, event, name)
    ...
    WowTracker:RegisterPlugin({
        ...
        -- Scan: currency scanner schrijft naar WowTrackerDB + DelveTrackerDB (compat)
        scan = function(charKey, charData, db)
            charData.currencies = charData.currencies or {}
            local ids = {3028, 3310, 3376, 3378, 3392, 3405}
            for _, id in ipairs(ids) do
                local ok, info = pcall(C_CurrencyInfo.GetCurrencyInfo, id)
                if ok and info then charData.currencies[id] = info.quantity end
            end
        end,

        -- buildUI: opent het bestaande XL Registry frame in de content area
        buildUI = function(contentFrame)
            local btn = CreateFrame("Button", nil, contentFrame, "BackdropTemplate")
            btn:SetSize(200, 36)
            btn:SetPoint("CENTER", contentFrame, "CENTER", 0, 60)
            btn:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8x8",edgeFile="Interface\\Buttons\\WHITE8x8",edgeSize=1})
            btn:SetBackdropColor(0.08, 0.03, 0.15, 1.0)
            btn:SetBackdropBorderColor(0.35, 0.0, 0.65, 0.7)
            local lbl = btn:CreateFontString(nil,"OVERLAY"); lbl:SetFont("Fonts\\2002.ttf",13,"OUTLINE")
            lbl:SetPoint("CENTER"); lbl:SetText("|cff00dfff📋 Open XL Registry|r")
            btn:SetScript("OnClick", function()
                if _G["DT_RegistryFrame"] then
                    if _G["DT_RegistryFrame"]:IsShown() then
                        _G["DT_RegistryFrame"]:Hide()
                    else
                        _G["DT_RegistryFrame"]:Show()
                    end
                end
            end)
            btn:SetScript("OnEnter",function(s) s:SetBackdropColor(0.15,0.05,0.25,1.0) end)
            btn:SetScript("OnLeave",function(s) s:SetBackdropColor(0.08,0.03,0.15,1.0) end)

            local info = contentFrame:CreateFontString(nil,"OVERLAY"); info:SetFont("Fonts\\2002.ttf",11,"OUTLINE")
            info:SetPoint("CENTER", contentFrame, "CENTER", 0, 18)
            info:SetText("|cff888888Currency scanner actief · data in WowTrackerDB|r")
        end,

        onEnable  = function() end,
        onDisable = function()
            if _G["DT_RegistryFrame"] then _G["DT_RegistryFrame"]:Hide() end
        end,
    })
end)
-- ============================================================
